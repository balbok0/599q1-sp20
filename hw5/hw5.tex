...
\begin{homeworkProblem}
    ...
    Note that we will prove it only for an error on second physical qubit, but due to symmetry, the correction also holds
    for errors on first and third registers.

    \[\Qcircuit @C=1em @R=1em {
        & \qw      & \gate{Z} \qwx[1] & \gate{Z} \qwx[2] & \qw              & \qw      & \qw \\
        & \qw      & \gate{Z}         & \qw              & \gate{Z} \qwx[1] & \qw      & \qw \\
        & \qw      & \qw              & \gate{Z}         & \gate{Z}         & \qw      & \qw \\
        & \gate{H} & \ctrl{-2}        & \qw              & \qw              & \gate{H} & \qw \\
        & \gate{H} & \qw              & \ctrl{-2}        & \qw              & \gate{H} & \qw \\
        & \gate{H} & \qw              & \qw              & \ctrl{-3}        & \gate{H} & \qw \\
    }\]

    We see then that for a state:
    ...
    \begin{itemize}
        ...
        \item With 1 error: $\hat{P}_1i = \bigotimes\limits_{j=0}^4 I(1 - \delta_{ij}) + Z\delta_{ij}$,
            where $i \in [0, 4]$
        \item With 2 errors: $\hat{P}_2ij = \bigotimes\limits_{k=0}^4 I(1 - \delta_{ik}\delta{jk}) + Z(\delta_{ik} + \delta_{jk})$,
...
\begin{homeworkProblem}
    ...
    This uses the fact that $S^4 = Z^2 = I$, and since $S^\dagger S = I$, then $S^\dagger = S^3$.
